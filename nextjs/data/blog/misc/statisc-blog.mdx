...
In my views, the Blog is to show articles and news. The most important thing is to make it easy to **read**. If we post our articles in something platform like the _CSDN_ or the _Zhihu_ etc. Those platforms is not clear, which have a lot advertisements, and much futures not support, like the $$\LaTeX$$.
So we need one way to show our articles, and we can control it. The one way is to build a blog by ourselves.
